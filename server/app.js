...
import { isVercelRuntime, saveUploadedFile, useBlobStorage } from './lib/uploads.js'
...
const storageConfig = useMemoryUploads
...
app.post('/api/orders/upload', authMiddleware, upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
